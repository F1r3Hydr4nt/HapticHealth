...
public class PlayerList : MonoBehaviour
{
	...
	public GameObject _button_newplayer;
	...
	{
		_player_canvas.active = false;
	}
	public void Show_panel()
	{
		_player_canvas.active = true;
		Refresh_panel();
	...
	public void Refresh_panel()
	{
		...
		_title.GetComponent<Text>().text = translations.getString ("players");
		_button_newplayer.GetComponent<Button>().GetComponentsInChildren<Text>()[0].text = translations.getString ("new_player");
		...
		List<Player> players = _database.getPlayers (team_id);
		...
		for (int i = 0; i < itemCount; i++)
		{
			...
			string name = players[i].name+" "+players[i].surname;
			int id = players[i].id;
			
			GameObject newItem = Instantiate(itemPrefab) as GameObject;
			newItem.name = "player_"+id.ToString();
		...
		_button_newplayer.GetComponent<Button>().onClick.AddListener(() =>
		                           {
			GlobalVariables.selected_player_id = 0;
			coach_and_train_scenario.GoToPlayerPersonalData();
