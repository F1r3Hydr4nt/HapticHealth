...
public class TeamList : MonoBehaviour
{
	...
	{
		_team_canvas.active = false;
	}
	public void Show_panel()
	{
		_team_canvas.active = true;
		Refresh_panel();
	...
	public void Refresh_panel()
	{
		...
		_title.GetComponent<Text>().text = translations.getString ("teams");
		...
		Debug.Log("Selected club: " + club_id);
		List<Team> teams = _database.getTeams(club_id);
		...
		for (int i = 0; i < itemCount; i++)
		{
			...
			string name = teams[i].name;
			int id = teams[i].id;
			int discipline_id = teams[i].discipline_id;

			GameObject newItem = Instantiate(itemPrefab) as GameObject;
			newItem.name = "team_"+id.ToString();
			...
			button.onClick.AddListener(() =>
			                                                   {
				GlobalVariables.selected_team_id = id;
