...
public class Child_ModalityList : MonoBehaviour
{
	/***
             * Description:
             ...
             *****/
	public Canvas _modality_canvas;
	...
	{
		_modality_canvas.active = false;
	}
	public void Show_panel()
	{
		_modality_canvas.active = true;
		Refresh_panel();
	...
	public void Refresh_panel()
	{
		...
		_title.GetComponent<Text>().text = translations.getString ("modalities");
		...
		Debug.Log("Modalities REFRESH PANEL");
		...
		List<Modality_Replay> modalities = _database.getModalities ();
		itemCount = modalities.Count;
		columnCount = 1;
		...
		for (int i = 0; i < itemCount; i++)
		{
			...
			string name = modalities[i].name;
			int id = modalities[i].id;
			
			GameObject newItem = Instantiate(itemPrefab) as GameObject;
			newItem.name = "modality_"+id.ToString();
			...
			button.onClick.AddListener(() =>
			                           {
				GlobalVariables.selected_modality_id = id;
				...
				play_and_learn_scenario.GoToDisciplines();
