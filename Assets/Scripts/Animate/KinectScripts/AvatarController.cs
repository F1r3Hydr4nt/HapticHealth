...
{	
    ...
    private List<Quaternion[]> _recorded_data;
    ...
    {	
        ...
        _recorded_data = new List<Quaternion[]>();
    ...
    public List<Quaternion[]> Stop_recording()
    {
        _record = false;
        Debug.Log("Record "+_recorded_data.Count+ " frames");
    ...
    {	
        ...
        if (_record)
        {
            _recorded_data.Add(new Quaternion[bones.Length]);
        }
	...
	protected void TransformBone(Int64 userId, KinectInterop.JointType joint, int boneIndex, bool flip)
    {
        ...
        if (_record)
        {
            _recorded_data[_recorded_data.Count - 1][boneIndex] = jointRotation;
        }
