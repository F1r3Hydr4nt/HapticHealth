...
public class KinectModelControllerV2 : MonoBehaviour {
	...
	void Start () {
		...
		_bones = new GameObject[(int)NuiSkeletonPositionIndex.Count + 5] {
		...
		_baseRotation = new Quaternion[(int)NuiSkeletonPositionIndex.Count];
		_boneDir = new Vector3[(int)NuiSkeletonPositionIndex.Count];
		...
		for( int ii = 0; ii < (int)NuiSkeletonPositionIndex.Count; ii++)
		{
			if((_nullMask & (uint)(1 << ii)) <= 0)
			{
				...
				if(ii % 4 == 3 && ((_nullMask & (uint)(1 << (ii/4) + (int)NuiSkeletonPositionIndex.Count)) <= 0))
				{
					_boneDir[ii] = _bones[(ii/4) + (int)NuiSkeletonPositionIndex.Count].transform.position - _bones[ii].transform.position;
				}
				//if the bone is the hip_override (at boneindex Hip_Left, get direction from average of left and right hips
				else if(ii == (int)NuiSkeletonPositionIndex.HipLeft && Hip_Left != null && Hip_Right != null)
				{
				...
				if(ii == (int)NuiSkeletonPositionIndex.Spine)
				{
		...
		_chestRight -= Vector3.Project(_chestRight, _boneDir[(int)NuiSkeletonPositionIndex.Spine]);
		//make _hipRight orthogonal to the direction of the hip override
		Vector3.OrthoNormalize(ref _boneDir[(int)NuiSkeletonPositionIndex.HipLeft],ref _hipRight);
	}
	...
	void RotateJoint(int bone) {
		...
		if(bone == (int)NuiSkeletonPositionIndex.Spine )
		{
		...
		if( sw.boneState[player,bone] == NuiSkeletonPositionTrackingState.NotTracked)
		{
			return;
		}
		...
		if(bone % 4 == 0)
		{
			...
			if(bone == (int)NuiSkeletonPositionIndex.HipLeft)
			{
				//target = vector from hip_center to average of hips left and right
				target = ((sw.bonePos[player,(int)NuiSkeletonPositionIndex.HipLeft] + sw.bonePos[player,(int)NuiSkeletonPositionIndex.HipRight]) / 2F) - sw.bonePos[player,(int)NuiSkeletonPositionIndex.HipCenter];
			}
			//otherwise it is one of the shoulders
			else
			{
				//target = vector from shoulder_center to bone
				target = sw.bonePos[player,bone] - sw.bonePos[player,(int)NuiSkeletonPositionIndex.ShoulderCenter];
			}
		}
		else
		{
		...
		if(bone == (int)NuiSkeletonPositionIndex.Spine)
		{
			...
			dir = _chestRight;
			target = sw.bonePos[player,(int)NuiSkeletonPositionIndex.ShoulderRight] - sw.bonePos[player,(int)NuiSkeletonPositionIndex.ShoulderLeft];
		...
		else if(bone == (int)NuiSkeletonPositionIndex.HipLeft)
		{
			//rotate the hips so they face forward (determined by the hips)
			dir = _hipRight;
			target = sw.bonePos[player,(int)NuiSkeletonPositionIndex.HipRight] - sw.bonePos[player,(int)NuiSkeletonPositionIndex.HipLeft];
		...
		if(bone == (int)NuiSkeletonPositionIndex.Spine)
		{
			restoreBone(_bones[(int)NuiSkeletonPositionIndex.HipLeft],_boneDir[(int)NuiSkeletonPositionIndex.HipLeft],upDir);
			restoreBone(_bones[(int)NuiSkeletonPositionIndex.HipLeft],_hipRight,rightDir);
		}
