...
public class SkeletonOneWrapper : MonoBehaviour {
	...
	[HideInInspector]
	public NuiSkeletonTrackingState[] players;
	[HideInInspector]
	...
	public NuiSkeletonPositionTrackingState[,] boneState;	
	...
	private void processSkeleton () {
		int[] tracked = new int[Constants.NuiSkeletonMaxTracked];
		tracked[0] = -1;
		tracked[1] = -1;
		int trackedCount = 0;
		//update players
		for (int ii = 0; ii < Constants.NuiSkeletonCount; ii++)
		{
			players[ii] = kinect.getSkeleton().SkeletonData[ii].eTrackingState;
			if (players[ii] == NuiSkeletonTrackingState.SkeletonTracked)
			{
		...
		for (int player = 0; player < 2; player++)
		{
			...
			{
				for (int bone = 0; bone < (int)NuiSkeletonPositionIndex.Count; bone++)
				{
					...
					NuiSkeletonBoneOrientation[] or = kinect.getBoneOrientations(kinect.getSkeleton().SkeletonData[trackedPlayers[player]]);
